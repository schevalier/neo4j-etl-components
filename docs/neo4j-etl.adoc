...
----
export NEO4J_HOME=/Users/mh/v/neo4j-enterprise-3.0.4

mkdir -p /tmp/northwind

./neo4j-etl mysql generate-metadata-mapping --database northwind --user root > /tmp/northwind/mapping.json
# Creating MySQL to CSV mappings...

echo '{"multiline-fields":"true"}' > /tmp/northwind/options.json

./neo4j-etl mysql export --database northwind --user root \
