...
* Exemplary integration into a 3rd party ETL pipeline (e.g. Nifi, Talend)

=== Who is it for

* Developer learning or playing with Neo4j for initial data import
* Partners providing data integration with Neo4j
* Enterprise developers building applicatiions based on well modeled relational data
...
=== Inferring Schema with Mapping Rules
...
* Build indexes automatically as part of import
* Missing ability to pass options to `neo4j-import` tool as a file instead of command line arguments
